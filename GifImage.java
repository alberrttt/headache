...
public class GifImage {
    ...
    /** public delay between each frame. */
    public int[] delay;
    /** public index of the current frame in the GIF file. */
    public int currentIndex;
    /** public time passed since the last frame in ms. */
    public long time;
    /** publicther the animation is paused or not. */
    public boolean pause;

    /**
     * Set the image of the actor. If the image is a normal picture, it will be
     * displayed as normal.
    ...
    /**
     * Copy the given GifImage. This is faster, and uses less memory, than loading
     * the same
     * GIF multiple times. The current play state (position in the GIF, paused
     * state) is copied
     * from the given GifImage, but after that they can be independently
     * played/paused.
     * 
     * The images making up the GIF are shared between the two images, so any
     * modifications to
     * the images will be shared in both GIFs. You can call this constructor on the
     * same source
     * GIF multiple times.
     * 
    ...
    /**
     * The Rectangle class represents rectangles. This is essentially a
     * re-implementation
     * of the java.awt.Rectangle class, created in order to avoid any dependency on
     * AWT.
     */
     ...
     * <br>
     * 
     * <i>I (Michael) edited this slightly on 10/09/08 to bring class up to date
     * with generics and therefore remove warnings.
     * Also edited so that resources are grabbed from the jar file and not
     * externally, so no security exceptions.</i>
     * <br>
     * <br>
     * 
     * <pre>
